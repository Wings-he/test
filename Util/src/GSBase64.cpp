			    ...
			    BOOL trimTrailingZeros)
{
  ...
  size_t resultBufSize = strlen(in) + 1;
  unsigned char* out = (unsigned char*) new char[resultBufSize]; // ensures we have enough space
  int k = 0;
  int paddingCount = 0;
...
unsigned char* GSBase64Decode(char const* pData, unsigned& nResultSize, BOOL bTrimTrailingZeros) 
{
	if (pData == NULL)
		return NULL; // sanity check

	return base64Decode(pData, strlen(pData), nResultSize, bTrimTrailingZeros);
}

char* GSBase64Encode(char const* pData, unsigned nSize) 
{
  unsigned origLength = nSize;
  unsigned char const* orig = (unsigned char const*)pData; // in case any input bytes have the MSB set
  if (orig == NULL)
	  return NULL;
...
void GSBase64FreeBuffer(const char * const pBuffer )
{
	delete[] pBuffer;
}
